...
VerKeL is a single Rails 8 monolith (Ruby 3.4.8, PostgreSQL, Yarn/Webpack + Sass). See `README.md` for the basics; this file covers non-obvious environment and workflow details.
...
When editing Ruby (`.rb`) or ERB (`.erb`) files, run `bin/rubocop` on the changed files and fix any new violations before committing (`bin/rubocop -a` autocorrects many of them). Keep style consistent with `.rubocop.yml`; don't churn unrelated pre-existing offenses.
