...
# Main-branch snapshot of {ArticlePackingPlanner}, kept for battle-test
# comparisons against the refactored planner.
class ArticlePackingPlannerLegacy
  ...
  def load_data
    @all_articles = Article.all.group_by(&:ingredient_unit).transform_values do |articles|
      articles.map { |a| ArticleAvailabilityPlannerLegacy.new(a) }
    end
